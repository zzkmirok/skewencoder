...
def plot_training_points(AE_df, skewness_df,iter_folder, iter):
    ...
    ax.scatter(AE_df['d1'],AE_df['d5'], color='blue', s=1, label='AE training data')
    # skewness_df.plot.scatter('p.x','p.y',s=1,ax=ax)
    ax.scatter(skewness_df['d1'],skewness_df['d5'], color='red', s=1, label='skewness training data')
...
def ae_visualize_sampling_all(simulation_folder, result_folder, iter, AE_df):
    data = load_dataframe(f'{simulation_folder}/COLVAR')
    data_ref= load_dataframe("./fes_rew.dat")
...
def check_if_in_which_basin(PREV_FOLDER_COLVAR):
    ...
    if switch[0] < 0.2 and switch[1] > 0.8:
        print("In basin 1")
        print("In basin 1")
        print("In basin 1")
        return 1
    elif switch[1] < 0.2 and switch[0] > 0.8:
        print("In basin 2")
        print("In basin 2")
        print("In basin 2")
        return 2
