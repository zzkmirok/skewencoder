...
parser.add_argument("-c", "--config", type=str, default="config.toml", help="Path to TOML config")
...
EXEC_DICT = {"mace": "/home/rwth1997/lammps-custom-mace/bin/lmp",
             "metatomic" : "/home/rwth1997/lammps-custom/bin/lmp"}

REPLACE_DICT = {
        "<SYSTEM_FILE>": os.path.join(CONFIG["simulation"]["systems_folder"], CONFIG["simulation"]["system"]+".data"),
        "<MLIP>": CONFIG["simulation"]["mlip"],
        "<MODEL_FILE>": os.path.join(CONFIG["simulation"]["model_folder"], CONFIG["simulation"]["model"]),
...
def descriptor_script():
    ...
    if CV_TYPE == "DISTANCE":
        for descr in DESCRIPTOR_LIST:
            ...
            if len(atom_nums) == 2:
                line = f"{descr}: {CV_TYPE} ATOMS={atom_nums[0]},{atom_nums[1]} {no_pbc}"
                lines.append(line)
...
def skewencoder_training(state_detection: STADECT.State_detection, iter: int, encoder_layers : Sequence[int], loss_coeff: float, batch_size: int):
...
{additional_biases()}
# PRINT all variables
PRINT FMT=%g STRIDE={CONFIG["plumed"]["stride"]} FILE={simulation_folder}/COLVAR ARG=dist73.x,dist77.x,dist73.y,dist77.y,dist73.z,dist77.z,{",".join(DESCRIPTOR_LIST)}
'''
...
def gen_plumed_biased(model_name : str,
                         file_path : str,
                         simulation_folder,
                         pos,
                         skew,
...
cv: PYTORCH_MODEL FILE={model_name} ARG={",".join(DESCRIPTOR_LIST)}
    '''
    print(input, file=file)
    file.close()
    walltype=""
...
PRINT FMT=%g STRIDE={CONFIG["plumed"]["stride"]} FILE={simulation_folder}/COLVAR ARG=dist73.x,dist77.x,dist73.y,dist77.y,dist73.z,dist77.z,{",".join(DESCRIPTOR_LIST)},cv.*""",file=f)
...
def loxodynamics_simulation(iter_folder, model_name, model, dataset, kappa, offset):
    ...
    gen_plumed_biased(model_name=model_name,
                         file_path=".",
...
def gen_input_lmp_template(lmp_file : str, replacements = REPLACE_DICT):
    with open(os.path.join(CONFIG["simulation"]["config_folder"], CONFIG["driver"], f"{lmp_file}.template"), "r") as f:
        template = f.read()
...
def main(kappa):
    ...
    if RANK == 0:
        ...
        subprocess.run([*bash_prefix,f"rm -rf {files_cleaned_pattern}"])#, cwd=SCRIPT_DIR)
        subprocess.run([*bash_prefix, f"mkdir -p {UNBIASED_FOLDER}"])

        subprocess.run([*bash_prefix, f"echo '******************************************************'"], cwd=SCRIPT_DIR)
        subprocess.run([*bash_prefix, f"echo Start {CONFIG["driver"]} {CONFIG["simulation"]["system"]} unbiased simulation"], cwd=SCRIPT_DIR)
        subprocess.run([*bash_prefix, f"echo '******************************************************'"], cwd=SCRIPT_DIR)
    
        gen_plumed_unbiased()

        subprocess.run([*bash_prefix,f"{job_exec} -k on g {n_gpus} -in in_unbiased.lammps -sf kk"], cwd=SCRIPT_DIR)
    ...
    if RANK == 0:
        ...
        subprocess.run([*bash_prefix, f"mv {CONFIG['simulation']["restart_file"]}.{n_steps} {CONFIG['simulation']["restart_file"]}"], cwd = SCRIPT_DIR)
        ...
        state_detection = STADECT.State_detection((threshold, 1-threshold), bond_type_dict=bond_type_dict, n_heavy_atom_pairs=n_descriptors)
    ...
    for iter in range(n_max_iter):
        if RANK == 0:
            state_detection, model, ITER_FOLDER, skewness_dataset, break_flag = skewencoder_training(state_detection, iter, encoder_layers, loss_coeff, batch_size)
        COMM.Barrier()
        ...
        if RANK == 0:
            ...
            loxodynamics_simulation(ITER_FOLDER, model_name, model, skewness_dataset, kappa, offset)
            subprocess.run([*bash_prefix,f"{job_exec} -k on g {n_gpus} -in in.lammps -sf kk"], cwd=SCRIPT_DIR)
            subprocess.run([*bash_prefix,f"cp ./plumed.dat {ITER_FOLDER}"], cwd=SCRIPT_DIR)
        
        COMM.Barrier()

        if RANK == 0:
            subprocess.run([*bash_prefix, f"mv {CONFIG['simulation']["restart_file"]}.{(iter+2)*n_steps} {CONFIG['simulation']["restart_file"]}"], cwd = SCRIPT_DIR)
            subprocess.run([*bash_prefix,f"rm -f {plumed_log}"], cwd=SCRIPT_DIR)
            subprocess.run([*bash_prefix, f"echo {CONFIG["driver"]} {CONFIG["simulation"]["system"]} simulation at iteration {iter} with plumed ends"], cwd=SCRIPT_DIR)
        COMM.Barrier()
