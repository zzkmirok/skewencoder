import os
# num_cpus_str = os.environ.get("SLURM_CPUS_PER_TASK", "1")
...
def main(kappa):
    ...
    n_cpus_per_lammps_task = CONFIG["simulation"]["n_cpus_per_lammps_task"]
    ...
    if not ARGS.test:
        job_exec = " ".join([f"srun --exact --ntasks=1 --cpus-per-task={n_cpus_per_lammps_task} --overlap", driver_exec])
    if RANK == 0:
        ...
        subprocess.run(
            [
                *bash_prefix,
                f"{job_exec} -k on g {n_gpus} t {n_cpus_per_lammps_task} -in in_unbiased.lammps -sf kk",
            ],
            cwd=SCRIPT_DIR,
        )
    ...
    for iter in range(n_max_iter):
        ...
        if RANK == 0:
            ...
            subprocess.run(
                [*bash_prefix, f"{job_exec} -k on g {n_gpus} t {n_cpus_per_lammps_task} -in in.lammps -sf kk"],
                cwd=SCRIPT_DIR,
            )
            subprocess.run(
