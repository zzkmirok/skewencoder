...
def gen_kappa_offset(iter, max_iter, kappa_all=4.0):
    ...
    kappa = 45 # smallest_kappa + (kappa_all - smallest_kappa) * iter / max_iter
...
def aes_run_plumed(iter, iter_folder, initial_position, nsteps, model, dataset, max_iter):
    ...
    gen_plumed_aes(model_name=f'model_autoencoder_{iter}.pt',
            ...
            potential_formula=ELOGATED_FORMULAR(),
            pos=mu_aes,
            skew=skew_aes,
            kappa=kappa,
            offset=offset)
...
def plot_training_points(AE_df, skewness_df,iter_folder, iter):
    ...
    plot_isolines_2D(elongated_potential,limits=((-10, 50),(-10, 50)),mode='contour',levels=np.linspace(0,120,12),ax=ax)
...
def ae_cv_isolines(model, n_components, iter_folder, iter, dataset):
    ...
    for i in range(n_components):
        ...
        plot_isolines_2D(elongated_potential,limits=((-10, 50),(-10, 50)), levels=np.linspace(0,120,8),mode='contour',ax=ax, linewidths=line_width-0.5)
        plot_isolines_2D(model, component=i,limits=((-10, 50),(-10, 50)), levels=25, colorbar=False, ax=ax, alpha=0.2,  linewidths=line_width-0.5)
        plot_isolines_2D(model, component=i, limits=((-10, 50),(-10, 50)),mode="scatter", levels=25, ax=ax, dataset=dataset, s=dot_size)
        plot_isolines_2D(model, component=i,limits=((-10, 50),(-10, 50)), mode='contour', levels=25, ax=ax, alpha=0.5, linewidths=line_width-0.5)
        ax.set_yticks([-10,10,30,50])
...
def ae_cv_wall(model, n_components, iter_folder, iter, dataset,max_iter):
    ...
    for i in range(n_components):
        ax = axs[i]
        # plot_isolines_2D(muller_brown_potential_three_states,levels=np.linspace(0,24,12),mode='contour',ax=ax)
        plot_isolines_2D(elongated_potential, limits=((-10, 50),(-10, 50)), levels=np.linspace(0,120,12),mode='contour',ax=ax)
        plot_2D_wall(model, kappa, pos, sign,component=i,limits=((-10, 50),(-10, 50)), levels=np.linspace(0,200,12), colorbar=True, ax=ax, alpha=0.8)
    # ax.set_title(f'CV isolines - {iter}')
...
def check_if_in_which_basin(PREV_FOLDER_COLVAR, tol = 2.0):
    position = np.average(load_dataframe(f'{PREV_FOLDER_COLVAR}').iloc[1500:][['p.x', 'p.y']].values, axis=0)
    basin1 = np.array([-0.31627709, 21.66011008])
    basin2 = np.array([21.43819785, -0.21754159]) 
...
def training_and_sampling_test(fun_name, max_iter, n_descriptors, n_components, loss_coeff, batch_size=0, use_all_data = True):
    ...
    for iter in range(max_iter):
        ...
        if iter == 0:
            initial_position = '0.0,20.0'
        else:
            initial_position = f"{last_conf[0]},{last_conf[1]}"
        last_conf, SIMULATION_FOLDER = aes_run_plumed(iter, ITER_FOLDER, initial_position=initial_position, nsteps=20000, model=model, dataset=skewness_dataset,max_iter=max_iter)
...
if __name__ == "__main__":
    ...
    training_and_sampling_test(fun_name, max_iter, n_descriptors, n_components, loss_coeff, batch_size, use_all_data = True)
