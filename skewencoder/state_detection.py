...
class Bond_type_lib:
    def __init__(self):
        self.bond_type_dict: Mapping[
            str, Mapping[str, Union[float, Tuple[int, int]]]
        ] = {}
...
def transform_colvar_key(
    colvar_key: str, pattern: str = r"^([A-Za-z]+)\d+([A-Za-z]+)\d+$"
) -> str:
...
def parse_unbiased_colvar(
    ...
    std_tol: float = 0.05,
    r0_tol: float = 1.6,
    transform_colvar_key: Callable[[str], str] = partial(
        transform_colvar_key, pattern=r"^([A-Za-z]+)\d+([A-Za-z]+)\d+$"
    ),
):
    colvar_df = load_dataframe(colvar_file)
    last_rows = colvar_df["time"].values.shape[0] // prop
    bond_type_dict: Mapping[
        str, Mapping[str, Union[float, Tuple[int, int]]]
    ] = {}  # TODO: should be bond type dict?
        ...
        if current_bond_type is not None:
            ...
            if current_bond_type not in bond_type_dict.keys():
                ...
                if current_std <= std_tol and current_r0 < r0_tol:
                    ...
                    if temp_r0 == r0_tol:
                        bond_type_dict[current_bond_type].update(
                            {"bond length": current_r0}
                        )
                    else:
    ...
    return (
        Bond_type_lib()(bond_type_dict=bond_type_dict),
        n_heavy_atom_pairs,
        heavy_atom_pairs_list,
    )
...
class State_detection:
    def __init__(
        ...
        bond_type_dict: Mapping[str, Mapping[str, Union[float, Tuple[int, int]]]],
        n_heavy_atom_pairs: int,
        ...
        self.states_connectivity: Sequence[
            np.ndarray
        ] = []  # TODO: Also sparse? So far 1-D array only heavy atoms
        ...
        for bond_name, options in self.bond_type_dict.items():
            ...
            for opt, opt_value in options.items():
                ...
                if opt == "m,n":
                    if not (
                        isinstance(opt_value, tuple)
                        and all(isinstance(i, int) for i in opt_value)
                    ):
                        raise TypeError(
                            f"Wrong type for option {opt} {type(opt_value)}"
                        )
                    else:
                        m = opt_value[0]
                        n = opt_value[1]
            ...
            if bond_length_boundary is None:
                ...
                temp_switch_fun = SwitchFun(
                    bond_length_boundary, options={"m": m, "n": n}
                )
    ...
    def __call__(self, colvar_file: str):  # TODO: directly colvar file?
        ...
        for key in colvar_df.keys():
            ...
            current_bond_type = transform_colvar_key(
                colvar_key=key, pattern=self.pattern
            )
            if current_bond_type is not None:
                ...
                print(
                    f"iter = {iter_key}, last_rows_mean={last_rows_mean}, key = {key}"
                )
                if (
                    last_rows_mean >= self.interval[0]
                    and last_rows_mean <= self.interval[1]
                ):
                    print(
                        f"Not Stable atoms pair:\n iter = {iter_key}, last_rows_mean={last_rows_mean}, key = {key}"
                    )
                    self.current_state = 0
...
def test_State_detection():
    ...
    test_state_detection = State_detection(
        (0.3, 0.7), bond_type_dict=bond_type_dict, n_heavy_atom_pairs=n_heavy_atom_pairs
    )
