...
from utils.plot import muller_brown_potential_three_states, muller_brown_potential, plot_isolines_2D, plot_metrics, paletteFessa
...
def MULLER_BROWN_FORMULAR():
    return '0.15*(146.7-280*exp(-15*(x-1)^2+0*(x-1)*(y-0)-10*(y-0)^2)-170*exp(-1*(x-0.2)^2+0*(x-0)*(y-0.5)-10*(y-0.5)^2)-170*exp(-6.5*(x+0.5)^2+11*(x+0.5)*(y-1.5)-6.5*(y-1.5)^2)+15*exp(0.7*(x+1)^2+0.6*(x+1)*(y-1)+0.7*(y-1)^2))'
def MULLER_BROWN_FORMULAR_1():
    return '0.15*(146.7-200*exp(-1*(x-1)^2+0*(x-1)*(y-0)-10*(y-0)^2)-100*exp(-1*(x-0)^2+0*(x-0)*(y-0.5)-10*(y-0.5)^2)-170*exp(-6.5*(x+0.5)^2+11*(x+0.5)*(y-1.5)-6.5*(y-1.5)^2)+15*exp(0.7*(x+1)^2+0.6*(x+1)*(y-1)+0.7*(y-1)^2))'
...
def gen_kappa_offset(iter, max_iter, kappa_all=4.0):
    ...
    kappa =  1.9# 2.0  # smallest_kappa + (kappa_all - smallest_kappa) * iter / max_iter
...
def aes_run_plumed(iter, iter_folder, initial_position, nsteps, model, dataset, max_iter):
    ...
    gen_plumed_aes(model_name=f'model_autoencoder_{iter}.pt',
            ...
            potential_formula=MULLER_BROWN_FORMULAR_1(),
            pos=mu_aes,
            skew=skew_aes,
            kappa=kappa,
            offset=offset)
...
def ae_cv_isolines(model, n_components, iter_folder, iter, dataset):
    ...
    for i in range(n_components):
        ...
        plot_isolines_2D(muller_brown_potential, levels=np.linspace(0,24,8),limits=((-1.8, 1.2), (-0.6, 2.4)), mode='contour',ax=ax, linewidths=line_width-0.5)
        plot_isolines_2D(model, component=i, limits=((-1.8, 1.2), (-0.6, 2.4)), levels=25, colorbar=False, ax=ax, alpha=0.2,  linewidths=line_width-0.5)
        plot_isolines_2D(model, component=i, limits=((-1.8, 1.2), (-0.6, 2.4)), mode="scatter", levels=25, ax=ax, dataset=dataset, s=dot_size)
        plot_isolines_2D(model, component=i, limits=((-1.8, 1.2), (-0.6, 2.4)), mode='contour', levels=25, ax=ax, alpha=0.5, linewidths=line_width-0.5)
        ax.set_yticks([-0.6, 0.4, 1.4, 2.4])
        ax.set_xticks([-1.8,-0.8,0.2,1.2])
...
def ae_cv_wall(model, n_components, iter_folder, iter, dataset,max_iter):
    ...
    for i in range(n_components):
        ...
        plot_2D_wall(model, kappa, pos, sign,component=i, levels=np.linspace(0,10,12), colorbar=True, ax=ax, alpha=0.8)
    # ax.set_title(f'CV isolines - {iter}')
...
def check_if_in_which_basin(PREV_FOLDER_COLVAR, tol = 0.3):
    # position = np.average(load_dataframe(f'{PREV_FOLDER_COLVAR}').iloc[1750:][['p.x', 'p.y']].values, axis=0)
    position = load_dataframe(f'{PREV_FOLDER_COLVAR}').iloc[-1][['p.x', 'p.y']].values
    basin1 = np.array([-0.55822364, 1.44172584])
    basin2 = np.array([-0.05001085, 0.4666941])
    basin3 = np.array([0.62349937, 0.02803775]) 
    distance1 = np.sqrt(np.sum((basin1 - position) ** 2))
    distance2 = np.sqrt(np.sum((basin2 - position) ** 2))
    distance3 = np.sqrt(np.sum((basin3 - position) ** 2))
    if np.min((distance1, distance2, distance3)) > tol:
        print("Not in any basin, keep warm starting")
        print("Not in any basin, keep warm starting")
        print("Not in any basin, keep warm starting")
        return 0
    else:
        if distance1 == np.min((distance1, distance2, distance3)):
            print("In basin 1")
            print("In basin 1")
            print("In basin 1")
            return 1
        elif distance2 == np.min((distance1, distance2, distance3)):
            print("In basin 2")
            print("In basin 2")
            print("In basin 2")
            return 2
