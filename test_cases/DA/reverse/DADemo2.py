...
def load_data(filenames_iter,filenames_all, multiple = 0, bs=0):
    ...
    AE_dataset, AE_df = create_dataset_from_files(filenames_all, return_dataframe=True, filter_args={'regex':'^d\d*'}, create_labels=False, verbose=False)
    ...
    skewness_dataset, skewness_df = create_dataset_from_files(filenames_iter, return_dataframe=True, filter_args={'regex':'^d\d*'}, create_labels=False, verbose=False)
...
def plot_training_points(AE_df, skewness_df,iter_folder, iter):
    ...
    d4 = np.array(data_ref.iloc[:]["d4"].values).reshape(101,101)
    d6 = np.array(data_ref.iloc[:]["d6"].values).reshape(101,101)
    fes = np.array(data_ref.iloc[:]["file.free"].values).reshape(101,101)
    fig,ax = plt.subplots(figsize=(4,4))
    ax.contour(d4, d6, fes, levels=np.linspace(0,150,15), linewidths=1, cmap="Greys_r")
    # plot_isolines_2D(muller_brown_potential_three_states,mode='contour',levels=np.linspace(0,24,12),ax=ax)
    # AE_df.plot.scatter('p.x','p.y',s=1,cmap='fessa',ax=ax)
    ax.scatter(AE_df['d4'], AE_df['d6'], color='blue', s=1, label='AE training data')
    # skewness_df.plot.scatter('p.x','p.y',s=1,ax=ax)
    ax.scatter(skewness_df['d4'],skewness_df['d6'], color='red', s=1, label='skewness training data')
...
def ae_visualize_sampling_all(simulation_folder, result_folder, iter, AE_df):
    ...
    d4 = np.array(data_ref.iloc[:]["d4"].values).reshape(100,100)
    d6 = np.array(data_ref.iloc[:]["d6"].values).reshape(100,100)
    fes = np.array(data_ref.iloc[:]["file.free"].values).reshape(100,100)
    fig, ax = plt.subplots(figsize=(4,3))
    ax.contour(d4, d6, fes, levels=np.linspace(0,50,10), linewidths=1, cmap="Greys_r")
    # data.plot.hexbin('p.x', 'p.y', C='opes.bias',cmap='fessa', ax=ax)
    ax.scatter(AE_df['d4'],AE_df['d6'], color='blue', s=1)
    ax.scatter(data.iloc[:]['d4'].values, data.iloc[:]['d6'].values, color='red', s=1)
...
def check_if_in_which_basin(PREV_FOLDER_COLVAR):
    ...
    if switch[0] < 0.2 and switch[1] < 0.2:
        print("In basin 1")
        print("In basin 1")
        print("In basin 1")
        return 2
    elif switch[1] > 0.8 and switch[0] > 0.8:
        print("In basin 2")
        print("In basin 2")
        print("In basin 2")
        return 1
...
def gradient_calculation(model, filenames_all):
    model.eval()
    current_dataset, current_df = create_dataset_from_files(filenames_all, return_dataframe=True, filter_args={'regex':'^d\d*'}, create_labels=False, verbose=False)
    filtered_df = current_df.filter(regex='^d\d*')
    current_data = filtered_df.values
    Inputs = current_dataset[:]['data'].clone().detach().requires_grad_(True)
    Outputs = model(Inputs)
    gradients = torch.autograd.grad(Outputs, Inputs, grad_outputs=torch.ones_like(Outputs), create_graph=True)
    gradient_sum = np.sum(np.abs(gradients[0].detach().numpy()), axis=0)
    variance= np.var(current_data, axis=0)
    ranking = np.multiply(gradient_sum, np.sqrt(variance))
    normalized_ranking = ranking / np.sum(ranking)
    # print(normalized_ranking[3], normalized_ranking[5], normalized_ranking[8], normalized_ranking[14])
...
def training_and_sampling(state, states, iter, n_descriptors, n_components, loss_coeff, batch_size=0, use_all_data = True):
    ...
    encoder_layers = [n_descriptors, 90, 40, 2, 20, n_components]
...
def Test_gradient_calculation(iter=20):
    filenames_all = [f"results/iter_{i}/data/COLVAR" for i in range(iter) ]
    filenames_all.append(f"unbiased/COLVAR")
    filenames_iter = [f"results/iter_{iter-1}/data/COLVAR"]
    batch_size=100
    AE_dataset, skewness_dataset, datamodule, AE_df, skewness_df = load_data(filenames_iter,filenames_all,multiple=(iter + 1), bs=batch_size)
    encoder_layers = [15, 90, 40, 2, 20, 1]
    loss_coeff = 0.1
    model = aes_model(AE_dataset,encoder_layers, loss_coeff, iter, PREV_ITER_FOLDER=f"results/iter_{iter}",)
    filenames_all.append(f"results/iter_{iter}/data/COLVAR")
    gradient_calculation(model, filenames_all)
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        terminate_iter = np.int_(sys.argv[1])
        for i in range(1,terminate_iter+1):
            Test_gradient_calculation(iter=i)
